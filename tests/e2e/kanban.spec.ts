...
test.describe('snelle invoer (quick capture)', () => {
  test('snelle invoer widget is zichtbaar op command-center', async ({ page }) => {
    await page.goto('/command-center')
    ...
    // Quick capture widget heeft een titel "Snelle invoer" of een input met placeholder
    await expect(page.getByText('Snelle invoer').or(page.getByPlaceholder(/Wat moet er gebeuren/))).toBeVisible()
  })

  test('kaart aanmaken via snelle invoer toont succes-toast', async ({ page }) => {
    await page.goto('/command-center')
    ...
    const titleInput = page.getByPlaceholder(/Wat moet er gebeuren/)
    if (await titleInput.count() === 0) {
      test.skip(true, 'Snelle invoer widget niet gevonden')
      return
    }

    const testTitle = `Test kaart ${Date.now()}`
    await titleInput.fill(testTitle)

    const submitButton = page.getByRole('button', { name: /Toevoegen/ })
    await submitButton.click()

    // Verwacht een succes-toast "Taak aangemaakt"
    await expect(page.getByText('Taak aangemaakt')).toBeVisible({ timeout: 5000 })
  })
})
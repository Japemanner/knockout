...
export function LocalDynamicTable({
  ...
  useEffect(() => {
    foreignKeys.forEach(async (fk) => {
      const result = await getLocalForeignKeyOptions({
  ...
  const loadPage = useCallback(async (pageNum: number) => {
    setLoading(true)
    const result = await getLocalTableRecords({ tableName, page: pageNum, pageSize })
    if (!result.error) {
      setRows(result.rows)
  ...
  const handleCreate = useCallback(async (values: Record<string, unknown>) => {
    const result = await createLocalRecord({ tableName, values })
    if (result.error) {
  ...
  const handleUpdate = useCallback(async (values: Record<string, unknown>) => {
    ...
    const result = await updateLocalRecord({
      tableName,
      primaryKey: { column: pk.name, value: row[pk.name] },
      values,
    })
    if (result.error) {
      toast({ title: 'Fout', description: result.error, variant: 'destructive' })
  ...
  }, [tableName, showForm, pk, page, loadPage, toast])

  const handleDelete = useCallback(async () => {
    if (!pk || !showDelete) return
    const result = await deleteLocalRecord({
      tableName,
      primaryKey: { column: pk.name, value: showDelete[pk.name] },
    })
    if (result.error) {
      toast({ title: 'Fout', description: result.error, variant: 'destructive' })
    } else {
    ...
    setShowDelete(null)
    loadPage(page)
  }, [tableName, showDelete, pk, page, loadPage, toast])
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-muted/50">
              ...
              {pk && <th className="px-3 py-2 w-20"></th>}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={displayColumns.length + (pk ? 1 : 0)} className="px-3 py-4 text-center text-muted-foreground">Laden...</td></tr>
            ) : rows.length === 0 ? (
              <tr><td colSpan={displayColumns.length + (pk ? 1 : 0)} className="px-3 py-4 text-center text-muted-foreground">Geen records</td></tr>
            ) : (
              rows.map((row, i) => (
